...
#func _init() -> void:
	#Gameboard.cells_changed.connect(
		#func _on_gameboard_cells_changed(cells: Array[Vector2i]) -> void:
			## Keep track of which cells have actually changed. Key = uid (int), value = coordinate
			## (Vector2i).
			#var added_cells: Dictionary[int, Vector2i] = {}
			#
			#for cell in cells:
				#var uid: = Gameboard.cell_to_index(cell)
				#if Gameboard.is_cell_clear(cell):
					#if not has_point(uid):
						#add_point(uid, cell)
					...
					#if has_point(uid):
						#remove_point(uid)
			#_connect_cells(added_cells)
	#)


func has_cell(cell: Vector2i) -> bool:
	return has_point(Gameboard.cell_to_index(cell))


# Connect any new cells with their neighbours whenever the pathfinder state changes.
# Old cells are automatically disconnected when they are removed via remove_point.
func connect_cells(added_cells: Dictionary[int, Vector2i]) -> void:
		...
		if has_point(uid):
			for neighbor in Gameboard.get_adjacent_cells(added_cells[uid]):
				...
				if has_point(neighbor_id):
					connect_points(uid, neighbor_id)
