...
##
## The gameboard is responsible for creating and maintaining the various game systems (e.g. the
## pathfinder, list of gamepieces, etc.).
...
func _ready() -> void:
	...
	cursor.initialize(grid, pathfinder.get_cells())
	...
	$DebugView.initialize(grid, gamepieces, _terrain.get_walkable_cells(grid.boundaries), 
